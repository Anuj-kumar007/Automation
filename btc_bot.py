#!/usr/bin/env python3
"""
BTC 5-Minute Polymarket Trading Bot – FULLY INTEGRATED
- Original prediction logic untouched
- Accuracy tracking and last 5 trades table
- Real balance display and order execution
"""
...
async def execute_trade(prediction, token_up, token_down, size, order_type="MARKET"):
    ...
    try:
        ...
        if resp and resp.get("success"):
            ...
            print(f"❌ Order FAILED: {resp.get('errorMsg', 'Unknown error')}", flush=True)
...
async def main():
    ...
    while True:
        try:
            ...
            if window_start_ts != last_window_start:
                if last_window_start is not None and last_prediction is not None:
                    ...
                    if end_price and price_to_beat:
                        ...
                        # If we had an active position, settle it in DB and show trade summary
                        if active_position:
            ...
            # Execute trade if confident and token IDs exist
