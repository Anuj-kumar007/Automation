...
for limit in LIMIT_PRICES:
    label = f"${limit:.2f}"
    accounts[label] = SimAccount(label, SIM_STARTING_BALANCE)

# -------------------------------------------------------------------
# BACKGROUND TASKS
# -------------------------------------------------------------------
async def btc_price_updater():
    """Continuously update BTC price every second using a persistent session."""
    async with aiohttp.ClientSession(headers=PUBLIC_HEADERS) as sess:
        while True:
            try:
                async with sess.get("https://api.binance.com/api/v3/ticker/price?symbol=BTCUSDT",
                                    timeout=3) as resp:
                    if resp.status == 200:
                        price = float((await resp.json())["price"])
                        market_data.update_btc(price)
            except Exception as e:
                print(f"⚠️ BTC price update error: {e}")
            await asyncio.sleep(1)

async def market_monitor(up_token, down_token, beat_price, stop_event, fill_events):
    """Monitor order book and fill events."""
            ...
            if prices:
                ...
                for label, limit in zip(accounts.keys(), LIMIT_PRICES):
                    ...
                    if ask_up is not None and ask_up <= limit and not evts["up"].is_set():
                        print(f"🎯 {label} UP fill triggered! ask_up={ask_up:.2f} ≤ {limit}")
                        evts["up"].set()
                    if ask_down is not None and ask_down <= limit and not evts["down"].is_set():
                        print(f"🎯 {label} DOWN fill triggered! ask_down={ask_down:.2f} ≤ {limit}")
                        evts["down"].set()
        except Exception as e:
...
<head>
    ...
    <title>Multi‑Limit Bot (BTC Live)</title>
    <style>
...
<body>
    <div class="dashboard">
        <h1>🧬 Multi‑Limit Bot (BTC Live)</h1>
        <div class="subtitle">Testing 6 limit prices · live ticker every 0.2s · BTC always fresh</div>
...
async def main():
    print("=" * 60)
    print("Multi‑Limit Bot – BTC always live, resilient")
    print("=" * 60)

    # Start the BTC price background task (runs forever)
    ...
    while True:
        try:
            ...
            fill_events = {}
            for label in accounts.keys():
                fill_events[label] = {"up": asyncio.Event(), "down": asyncio.Event()}

            stop_event = asyncio.Event()
            monitor_task = asyncio.create_task(
                market_monitor(token_up, token_down, beat_price, stop_event, fill_events)
            )
            ...
            for label, limit in zip(accounts.keys(), LIMIT_PRICES):
                evts = fill_events[label]
                fill_status[label] = (evts["up"].is_set(), evts["down"].is_set())
            ...
            print(f"Settlement: {actual} (settle ${settlement_price:.2f}, beat ${beat_price:.2f})")
            ...
            for label, limit in zip(accounts.keys(), LIMIT_PRICES):
                ...
                acc = accounts[label]
                acc.update(pnl, sides_filled, win)
                acc.add_trade(ws, {
                ...
                print(f"  {label}: {'UP='+str(filled_up)+' DOWN='+str(filled_down)} P&L ${pnl:.2f} | Bal ${acc.balance:.2f}")

            await asyncio.sleep(2)

        except Exception as e:
            print(f"❌ Window error: {e} – skipping and continuing in 5s")
