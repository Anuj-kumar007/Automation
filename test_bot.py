            ...
            async with session.get(url, timeout=15) as resp:
                if resp.status == 200:
                    ...
                    if len(token_ids) >= 2:
                        ...
                        print_test("Gamma API", False, "No token IDs")
                elif resp.status == 404:
                    print_test("Gamma API", True, f"Market not found (connection OK)")
                    gamma_ok = True
                else:
...
async def test_clob_auth():
    ...
    try:
        ...
        print_test("CLOB Client Import", False, "Missing py-clob-client")
        return False
...
def test_database():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("CREATE TABLE trades (id INTEGER PRIMARY KEY, prediction TEXT, size REAL, fill_price REAL)")
        cursor.execute("CREATE TABLE settlements (id INTEGER PRIMARY KEY, trade_id INTEGER, pnl_usdc REAL)")
        print_test("Table Creation", True, "trades and settlements tables created")
        cursor.execute("INSERT INTO trades (prediction, size, fill_price) VALUES (?, ?, ?)", ("UP", 5.0, 0.67))
        trade_id = cursor.lastrowid
        conn.commit()
        cursor.execute("SELECT * FROM trades WHERE id = ?", (trade_id,))
        print_test("Data Insert", cursor.fetchone() is not None, f"Inserted trade ID: {trade_id}")
        cursor.execute("INSERT INTO settlements (trade_id, pnl_usdc) VALUES (?, ?)", (trade_id, 1.65))
        conn.commit()
        cursor.execute("SELECT t.prediction, t.size, t.fill_price, s.pnl_usdc FROM trades t LEFT JOIN settlements s ON t.id = s.trade_id WHERE t.id = ?", (trade_id,))
        row = cursor.fetchone()
...
async def test_price_fetching():
    ...
    past_ts = int(datetime.now(timezone.utc).timestamp()) - 300
...
def test_order_validation():
    ...
    for order_type, input_size, est_price, expected in test_cases:
        ...
        print(f"   {'✅' if passed else '❌'} {order_type}: {input_size} shares @ ${est_price} → {size} shares")
        if not passed:
            all_passed = False
    return all_passed
...
def test_window_timing():
    ...
    test_times = [
        datetime(2024, 1, 15, 10, 13, 30),
        datetime(2024, 1, 15, 10, 17, 45),
        datetime(2024, 1, 15, 10, 20, 0),
        datetime(2024, 1, 15, 10, 22, 15),
    ]
    all_passed = True
    for dt in test_times:
        ...
        passed = window_start.minute == window_minute
        print(f"   {'✅' if passed else '❌'} {dt.strftime('%H:%M:%S')} → Window: {window_start.strftime('%H:%M')} - {window_end.strftime('%H:%M')}")
        if not passed:
            all_passed = False
    return all_passed
