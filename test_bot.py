...
# ============================================================
# Create a reusable SSL connector (ignore certs for Termux)
# ============================================================
...
async def test_connectivity():
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        try:
            # Use a known recent market slug
            ...
            async with session.get(url, timeout=15) as resp:
                if resp.status == 200:
                    ...
                    print_test("Gamma API", True, f"Market not found (expected for past/future): {slug}")
                    gamma_ok = True  # 404 is acceptable, connection works
                else:
